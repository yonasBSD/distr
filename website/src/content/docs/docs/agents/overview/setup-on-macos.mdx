...
slug: docs/agents/distr-on-macos
sidebar:
  order: 6
---
...
<Aside type="note">
  ...
  the [Distr agents](/docs/agents/). Since Distr can also be self-hosted, much
  of this guide could apply to running Distr in a local Docker environment as
  well. However, we'll focus on the SaaS version of Distr and properly
  configuring a local MacOS target environment to deploy applications through
  Distr.
</Aside>
...
To verify everything is configured correctly, try [creating a deployment](/docs/agents/deployment/) through Distr.
...
### 1. Metrics collection must be disabled

Metrics collection is not supported on macOS due to Docker Desktop's virtualized filesystem not supporting the required mount propagation.
When creating or editing a deployment target for macOS, make sure to **disable metrics collection** in the deployment target settings.

### 2. Log collection does not work

Log collection is not supported on macOS.
