...
description: Store sensitive configuration values centrally in Distr and reference them in Docker and Kubernetes deployments using template syntax.
slug: docs/agents/secrets
sidebar:
...
<Aside type="note">
  This page covers Distr's built-in secret store, which works for both Docker
  and Kubernetes deployments. If you need secrets delivered as **files inside
  containers** rather than as environment variables, see [Docker Compose
  Secrets](/docs/agents/docker-compose-secrets/) — a Docker-native mechanism
  that can be used on its own or combined with Distr Secrets.
</Aside>
...
If any active deployments reference this secret, Distr will show you which deployment targets are affected before saving.
...
As a customer, you can create and manage secrets for your own organization from the Customer Portal. For the full customer-side guide, see [Customer Portal Secrets](/docs/platform/customer-portal/secrets/).
